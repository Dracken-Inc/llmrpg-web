...
export class NarrativeSystem extends System {
  ...
  private eventToNarrative(event: NarrativeEvent): string {
  ...
  getRecentNarrative(seconds: number = 60): NarrativeEntry[] {
    const now = Date.now();
    const cutoff = now - seconds * 1000;
    return this.narrativeLog.filter((entry) => entry.timestamp > cutoff);
  }

  formatNarrativeForDisplay(maxEntries: number = 20): string {
